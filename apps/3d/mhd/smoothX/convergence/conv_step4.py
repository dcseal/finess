...
for parameters_ini_filename in parameters_ini_filename_list:
    ...
    qexact[:, :, :, 1 - 1] = 1.0
    qexact[:, :, :, 2 - 1] = 0.0
    qexact[:, :, :, 3 - 1] = 0.1 * sin(2.0*pi * (X+t))
    qexact[:, :, :, 4 - 1] = 0.1 * cos(2.0*pi * (X+t))
    qexact[:, :, :, 6 - 1] = 1.0
    qexact[:, :, :, 7 - 1] = 0.1 * sin(2.0*pi * (X+t))
    qexact[:, :, :, 8 - 1] = 0.1 * cos(2.0*pi * (X+t))
    pexact = 0.1
    # We don't care about energy, because we don't test the accuracy
    # of that.
    # qexact[:, :, :, 5 - 1]
    #L1_error = sum(abs(q[:,:,:, 3 - 1]/q[:,:,:, 1 - 1] - qexact[:,:,:, 3 - 1]/qexact[:,:,:, 1 - 1])) / sum(abs(qexact[:,:,:, 3 - 1]/qexact[:,:,:, 1 - 1]))   #u2
    #L1_error = sum(abs(q[:,:,:, 4 - 1]/q[:,:,:, 1 - 1] - qexact[:,:,:, 4 - 1]/qexact[:,:,:, 1 - 1])) / sum(abs(qexact[:,:,:, 4 - 1]/qexact[:,:,:, 1 - 1]))   #u3
    #L1_error = sum(abs(q[:,:,:, 7 - 1] - qexact[:,:,:, 7 - 1])) / sum(abs(qexact[:,:,:, 7 - 1]))  #B2
