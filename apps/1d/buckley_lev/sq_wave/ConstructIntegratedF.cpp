    ...
    dTensorBC1& smax, dTensorBC2& F)
{
    ...
    dTensorBC2 f_x (mx, meqn, mbc );
    dTensorBC2 f_xx(mx, meqn, mbc );
...
#pragma omp parallel for
    for( int i = 1-mbc_small; i <= mx+mbc_small; i++ )
    {
        ...
        for( int m1=1; m1 <= meqn; m1++ )
        {
            ...
            {
                tmp += -A.get(1, m1, m2) * f_x.get(i,m2);
            }
            f_t.set( m1, tmp );
        }
        ...
        if( global_ini_params.get_time_order() > 2 )
        {
            ...
            for( int m =1; m <= meqn; m++ )
            {
                ...
                for( int m2=1; m2 <= meqn; m2++ )
                {
                    tmp1 += H.get(1,m,m1,m2) * f_x.get(i,m1)*f_x.get(i,m2);
                    tmp2 += H.get(1,m,m1,m2) * qx_val.get(m1)*f_x.get(i,m2);
                }
                f_tt.set( m, tmp1 );
                tmp_vec.set( m, tmp2 );
            }
