...
void Diff1( double dx, const dTensor2& f, dTensor1& fx )
{
    ...
    for( int m=1; m <= mcomps; m++ )
    {
        ...
        fx.set( m, tmp / dx );
//      fx.set( m, ( f.get(m,4) - f.get(m,2) ) / (2.0*dx) );
    }
...
// Central (non-conservative) finite differences based on WENO smoothness
// indicators.

void Diff1( double dx, const dTensor2& g, dTensor1& fx )
{
...
//      double tmp1 = (  g.get( m, 1 ) - g.get( m, 5 ) )*(1.0/12.0);
//      tmp1       += ( -g.get( m, 2 ) + g.get( m, 4 ) )*(2.0/ 3.0);
//      fx.set( m, tmp1 / dx );

        // second-order (linear) approximation:
//      double tmp2 = (  0.5*( g.get( m, 4 ) - g.get( m, 2 ) ) );
//      fx.set( m, tmp2 / dx );
...
void Diff2( double dx, const dTensor2& g, dTensor1& fxx )
{
...
//      double tmp = ( -g.get( m, 1 ) - g.get( m, 5 ) )*(1.0/12.0);
//      tmp       += (  g.get( m, 2 ) + g.get( m, 4 ) )*(4.0/ 3.0);
//      tmp       += ( -g.get( m, 3 )                 )*(5.0/ 2.0);
//      fxx.set( m, tmp / (dx*dx) );

        // # Return 4th-order approximation to derivative
        fxx.set(m, ( (omt0*u1 + omt1*u2 + omt2*u3)/omts) / (dx*dx) );
//      fxx.set(m, ( (g0*u1 + g1*u2 + g2*u3)) / (dx*dx) );
