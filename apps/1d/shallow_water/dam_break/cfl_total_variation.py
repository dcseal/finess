...
'''
    ...
    print("Warning: This problem assumes maximum height = 3.0, and minimum height = 1.0")
    print("-----------------------------------------------------------------------")
...
#       data = np.loadtxt( folder + '/conservation.dat' )
        ...
        print("%2.15e;    %2.5e;    %2.5e;    %2.5e" % ( cfl_now, dtv, min(minq-1.0,0.), max(maxq-3.0,0.) ) )
        fids.write( "%2.3f    %2.15e    %2.5e    %2.5e\n" % ( cfl_now, dtv, min(minq-1.0,0.), max(maxq-3.0,0.) ) )
    ...
    if( plt_soln ):
        ...
        ax1.set_title('Shallow Water Equations')
