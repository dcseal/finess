...
void FinSolveLxW( StateVars& Qnew, double tend, double dtv[] )
{
    ...
    dTensorBC3	 tmpFG(mx, my, meqn, mbc );
    ...
    while( t<tend )
    {
        ...
        while( m_accept==0 )
        {
            ...
            BeforeStep(dt, Qnew);
            //SetBndValues(Qnew);
            // To make sure we use the right boundary condition, we have to
            // apply boundary conditions twice here. This part will slow down
            // the code. -QT
			SetBndValuesX(Qnew);
            ConstructIntegratedR( dt, Qnew, smax, F, tmpFG);
	  	    SetBndValuesY(Qnew);
		    ConstructIntegratedR( dt, Qnew, smax, tmpFG, G);

            if( global_ini_params.get_mpp_limiter() )
            {

                // Construct the high-order flux
                ConstructLxWL_BCx2( Qnew, F, G, *fhat, *ghat, Lstar, smax );
       
                // Construct the low-order flux
                ConstructLFL_BCx2( dt, Qnew, *fLF, *gLF, Lstar, smax );
