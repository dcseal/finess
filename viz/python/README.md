...
To execute a plotting routine, type
    ...
    $ python $FINESS/viz/python/plotdog[n].py
...
For a full list of options, type
        ...
        $ python $FINESS/viz/python/plotdog[n].py -h

For example, running


        $ python $FINESS/viz/python/plotdog1.py -p 4 -o output3000
...
is plotted.  For example, these scripts have the following format:
    ...
    plotq1       - 1D script
    plotq2\_cart - 2D, Cartesian script     (not yet supported in FINESS)
    plotq2\_unst - 2D, Unstructured script  (not supported in FINESS)
